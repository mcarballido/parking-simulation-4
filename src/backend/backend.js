...
new TrabajoPractico().comenzarEjecucion()
      ...
      return {
        ...
        autos: datos.autosIngresados.map(auto => ({ nro: auto.nro, estado: auto.estado })),
        filaCaja: datos.filaCaja.map(auto => ({ nro: auto.nro })),
        utilitariosParcialmenteLibres,
        utilitariosLibres,
        utilitariosOcupados,
        grandesLibres,
        grandesOcupados,
...
app.get('/api/simulacion', (req, res) => {
  const trabajoPractico = new TrabajoPractico();
  const datos = trabajoPractico.comenzarEjecucion(); // Asumimos que esta función retorna los datos necesarios
